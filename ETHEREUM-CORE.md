...
| **Revocation is instant and key-free**    | Per-agent grants carry `expires_at`, `permissions`, and `secret_path_pattern`; revocation is a single row update. Bearer tokens (`mcpk_live_*`) are constant-time-checked against Postgres and can be rotated independently. Revocation never requires touching the keys themselves.                                       |
| **One pattern, every Ethereum environment** | The same MCP surface, custody model, and grant logic apply on every chain in `viem/chains` — Mainnet, Sepolia, Arbitrum, Optimism, Base, Gnosis, Polygon. No per-rollup integration; an agent fluent in Orloj is fluent in every Ethereum environment without re-tooling.                                                |
